...
- `EXTCODESIZE` is replaced with `ISCONTRACT`, described below. 
...
The former `EXTCODESIZE` opcode is replaced with a new opcode `ISCONTRACT`. The EOF opcode behaves in exactly the same way as the legacy opcode with the exception that any non-zero value pushed onto the stack is replaced with pushing a value of one, and any account without code will push a value of zero on the stack. Gas costs will be the same as for `EXTCODESIZE`, and any future gas schedule changes to `EXTCODESIZE` will also apply to `ISCONTRACT`. The opcode will also enforce the same Address Space Extension checks the [EIP-7069](./eip-7069.md) opcodes enforce.

* Opcode - `0x3B`
* Name - `ISCONTRACT`
* Gas cost - As per [EIP-2929](./eip-2929.md), 2600 gas if the account is not in the `accessed_addresses` list, 100 gas if the account is in the list.
* Execution - 
  * Pop one value off the stack, which is an address. 
  * If the address has any of the high 12 bytes set to a non-zero value exceptionally halt. 
  * Add the address to the EIP-2929 `accessed_addresses` list.
  * If the account at that address does not exist, or if the account has zero length code push 0 on the stack, otherwise push one onto the stack. 
  * Whether the target contract code is EOF or legacy does not matter to the execution.
...
### Replacing `EXTCODESIZE` with `ISCONTRACT`

The  `ISCONTRACT` opcode is needed because there are some critical use-cases where distinguishing EOAs from contract accounts is essential. Namely, [ERC-721](./eip-721.md)'s `safeTransferFrom` function behaves differently if an account is a contract, and it specifies that code size greater than zero is the way to differentiate.

Simply restoring the `EXTCODESIZE` operation creates the possibility that code introspection could be compromised by knowing the size of the deployed code. If the operation instead becomes a boolean operation then the code utility can be preserved while also reducing the impact on the "remove code introspection" mandate.

Generally speaking, EOF aims to avoid replacing opcodes in-place when semantics change. This case, however, mimics the `PREVRANDAO` situation, in that the core utility of the opcode is preserved and the behavior is a drop-in replacement for existing code. Note that there are no existing contracts that would be impacted by this either as a new or replacement opcode.
