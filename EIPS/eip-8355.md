...
In the long term, aggregating post-quantum signatures into a single proof
verifies them more cheaply than this precompile does. Some applications will
still opt out of aggregation, whether sensitive to proving latency, bound by a
compliance regime that requires checking the signature as issued, or
integrating an existing signing workflow where operating a prover costs more
than the gas it saves; pricing verification as an ordinary precompile keeps
that path open. In the meantime, it lets users who are not gas-sensitive
migrate to a post-quantum account before aggregation is available.
