...
While *worklist* is not empty, pop an instruction and:
...
The time and memory complexity if this pass is *O(len(code))*. Each instruction is visited at most once.
...
The operand stack validation algorithm ignores *unreachable* instructions. The algorithm can beextended to reject any code having any unreachable instructions but additional instructions traversal is needed to search for unvisited instructions (or more efficient algorithm must be developed).
